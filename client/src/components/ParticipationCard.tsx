...
export default function ParticipationCard({
  ...
  setParticipationToRate?: (id: number) => void;
}) {
  ...
  return (
    <div className={`${styles.completionCard} neon-border-dual`}>
      ...
      <div className={styles.cardContent}>
        ...
        <StarRating rating={participation.averageParticipationScore} readOnly />

        {isLoggedIn && (
          <button
            className={styles.rateButton}
            onClick={() => setParticipationToRate?.(participation.id)}
            disabled={!setParticipationToRate}
          >
            {setParticipationToRate ? "Noter cette participation" : "Déjà noté"}
          </button>
        )}
      </div>
    </div>
